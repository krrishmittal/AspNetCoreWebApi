...
namespace GoogleLogin.Controllers
{
    ...
    public class UserAPIController : ControllerBase
    {
        ...
        private int GetCurrentUserId()
        {
            ...
            // Check if the claim value is already a plain integer (from cookie auth)
            // or if it's encrypted (from JWT auth)
            if (int.TryParse(userIdClaim.Value, out int userId))
            {
                // Plain integer from cookie authentication
                return userId;
            }

            // Encrypted value from JWT authentication - decrypt it
            try
            {
